...
const SC = require('./search_core.js');
const fs = require('fs');
...
/* ═══════════════════ STEP 4 — MINIMAX GROUND TRUTH ═════════════════════ */

/* Independent hand oracle for fixture A1F (§3.1).
   Hand-checked by Codex and independently re-derived from the §3–§4 rules
   during Step 4 review. This table is LITERAL DATA — it is not generated by
   minimax() and must never be regenerated from it. Do not edit without a
   fresh hand derivation.

   Root MAX action | worst-case value | subtree nodes | canonical MIN reply
   MOVE N          |               −3 |            36 | ROAD_CLOSE 0,1|0,2
   MOVE E          |               +4 |            42 | ROAD_CLOSE 0,1|0,2
   WAIT            |               −3 |            43 | ROAD_CLOSE 0,1|0,2  */
const A1F_HAND_ORACLE = {
    rootTable: [
        { action: { type: 'MOVE', dir: 'N' }, value: -3, subtreeNodes: 36,
          minReply: { type: 'ROAD_CLOSE', edge: '0,1|0,2' } },
        { action: { type: 'MOVE', dir: 'E' }, value: 4, subtreeNodes: 42,
          minReply: { type: 'ROAD_CLOSE', edge: '0,1|0,2' } },
        { action: { type: 'WAIT' }, value: -3, subtreeNodes: 43,
          minReply: { type: 'ROAD_CLOSE', edge: '0,1|0,2' } },
    ],
    rootValue: 4,
    rootAction: { type: 'MOVE', dir: 'E' },
    treeNodes: 122,                        /* 1 + 36 + 42 + 43 */
    pv: [
        { type: 'MOVE', dir: 'E' },
        { type: 'ROAD_CLOSE', edge: '0,1|0,2' },
        { type: 'MOVE', dir: 'N' },
        { type: 'PASS' },
        { type: 'SERVE' },
    ],
...
function suiteMinimax() {
    console.log('══ STEP 4 MINIMAX GATE ══');

    section('M1 hand-oracle root-action table (literal, not generated)');
    for (const row of A1F_HAND_ORACLE.rootTable) {
        const child = E.apply(E.initState('A1F'), row.action);
        const r = SC.minimax(child);
        const tag = row.action.type + (row.action.dir ? ' ' + row.action.dir : '');
        eq(r.value, row.value, 'M1 ' + tag + ' worst-case value');
        eq(r.nodes, row.subtreeNodes, 'M1 ' + tag + ' subtree node count');
        eq(r.action, row.minReply, 'M1 ' + tag + ' canonical MIN reply');
    }

    section('M2 root result: action, value, PV, node count');
    const root = SC.minimax(E.initState('A1F'));
    eq(root.action, A1F_HAND_ORACLE.rootAction, 'M2 root action = MOVE E');
    eq(root.value, A1F_HAND_ORACLE.rootValue, 'M2 root value = +4');
    eq(root.nodes, A1F_HAND_ORACLE.treeNodes,
       'M2 exact complete-tree node count = 122 (§3.1 gate: ≤200)');
    ok(root.nodes <= 200, 'M2 §3.1 gate: node count ≤ 200');
    eq(root.pv, A1F_HAND_ORACLE.pv, 'M2 full alternating PV (MIN and PASS included)');
    eq(root.pv.filter((_, i) => i % 2 === 0), A1F_HAND_ORACLE.pvMaxOnly,
       'M2 MAX-only teaching projection = MOVE E → MOVE N → SERVE');

    section('M3 MAX maximizes, MIN minimizes (hand-built/reachable states)');
    const afterN = E.apply(E.initState('A1F'), A.M('N'));   /* MIN to move */
    const vRC = SC.minimax(E.apply(afterN, A.RC('0,1|0,2'))).value;
    const vPASS = SC.minimax(E.apply(afterN, A.P)).value;
    eq([vRC, vPASS], [-3, 4], 'M3 MIN children after MOVE N: RC → −3, PASS → +4');
    eq(SC.minimax(afterN).action, A.RC('0,1|0,2'),
       'M3 MIN selects the smaller (RC, −3) — never PASS (+4)');
    const atQ = E.apply(E.apply(E.apply(E.initState('A1F'),
        A.M('E')), A.RC('0,1|0,2')), A.M('N'));            /* on H_A tile */
    const atQmax = craft(atQ, { toMove: 'MAX' });
    const rQ = SC.minimax(atQmax);
    eq([rQ.action, rQ.value], [A.S, 4],
       'M3 MAX on H_A tile selects SERVE (+4) over WAIT (−3)');

    section('M4 canonical tie-breaking (strict > / strict <, first kept)');
    const afterE = E.apply(E.initState('A1F'), A.M('E'));   /* MIN to move */
    const eRC = SC.minimax(E.apply(afterE, A.RC('0,1|0,2'))).value;
    const ePASS = SC.minimax(E.apply(afterE, A.P)).value;
    eq([eRC, ePASS], [4, 4], 'M4 MIN children after MOVE E tie at +4');
    eq(SC.minimax(afterE).action, A.RC('0,1|0,2'),
       'M4 MIN tie → first canonical action (ROAD_CLOSE before PASS)');
    const lastPly = craft(E.initState('A1F'), { t: 2 });    /* MAX, all → −3 */
    const rTie = SC.minimax(lastPly);
    eq([rTie.action, rTie.value], [A.M('N'), -3],
       'M4 MAX three-way tie at −3 → first canonical action (MOVE N)');
    eq(rTie.nodes, 4, 'M4 tie state: 1 root + 3 leaves = 4 nodes');
    eq(rTie.pv, [A.M('N')], 'M4 tie state PV is the single kept ply');

    section('M5 terminal-state return contract');
    const t5 = craft(E.initState('A1F'), { t: 3 });
    eq(SC.minimax(t5), { action: null, value: -3, nodes: 1, pv: [] },
       'M5 t=T terminal → {null, utility, 1, []}');
    const s5 = craft(E.initState('A1F'), { t: 2 });
    s5.households.forEach(h => { h.served = true; });
    eq(SC.minimax(s5), { action: null, value: 5, nodes: 1, pv: [] },
       'M5 all-served terminal → utility 4+1=5, nodes 1, empty PV');

    section('M6 determinism & purity');
    eq(JSON.stringify(SC.minimax(E.initState('A1F'))),
       JSON.stringify(SC.minimax(E.initState('A1F'))),
       'M6 repeated runs byte-identical');
    const in6 = E.initState('A1F');
    const snap6 = JSON.stringify(in6);
    SC.minimax(in6);
    eq(JSON.stringify(in6), snap6, 'M6 minimax does not mutate its input');
    const rFrozen = SC.minimax(E.A1F);                      /* deep-frozen fixture */
    eq(rFrozen.value, 4, 'M6 runs on the frozen exported fixture');
    const src4 = fs.readFileSync(path.join(__dirname, 'search_core.js'), 'utf8');
    ok(!/Math\.random|Date\.now|new Date|document\.|window\.|localStorage/.test(src4),
       'M6 search_core source static audit: RNG-free, clock-free, DOM-free');
}

/* ═══════════════════ RUN ═══════════════════════════════════════════════ */
const SUITES = { unit: suiteUnit, invariants: suiteInvariants, minimax: suiteMinimax };
const VALID = Object.keys(SUITES).concat(['all']);
const mode = (process.argv[2] || 'all').toLowerCase();
if (VALID.indexOf(mode) === -1) {
    console.error('Unknown suite "' + process.argv[2] + '". Valid: ' + VALID.join(', '));
    process.exit(2);
}
for (const name of Object.keys(SUITES))
    if (mode === name || mode === 'all') SUITES[name]();

console.log('──────────────────────────────────────────');
if (passed + failed === 0) {
    console.error('No assertions ran — harness misconfiguration.');
    process.exit(2);
} else if (failed === 0) {
